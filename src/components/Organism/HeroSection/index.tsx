import { BadgeDollarSign, ChevronDown, Eye, Mouse, Phone, Users } from "lucide-react";
import Button from "../../Atoms/Button";

export default function HeroSection() {
  return (
    <section className="pt-32 pb-4 md:px-4 relative flex items-center">
      {/* Left Side Animation Circle */}
      ...
      <div className="mx-auto text-center fade-in">
        <h1 className="flex flex-col text-5xl md:text-7xl font-bold mb-6 px-2 md:px-0">
          <span className="block text-black dark:text-white">From Concept to Code</span>
        ...
        <Button className="rounded-full btn-shadow flex items-center gap-2 mx-auto"><Phone size={20} />Let's have a call</Button>

        <div className="w-full flex flex-col items-center justify-center mt-12 animate-bounce">
          <Mouse className="text-red-500 dark:text-white" />
          <hr className="w-[1px] h-[8rem] dark:bg-white bg-red-500 mt-1" />
          <ChevronDown size={22} className="-mt-3 text-red-500 dark:text-white" />
        </div>
      </div>
